...
export interface ExitButtonConfig {
  ...
  /** URL to redirect re-engaged users to (defaults to "/") */
  redirectUrl?: string;
  /** URL to redirect churned users to (e.g. "/cancel-confirmed"). If not set, modal just closes. */
  churnRedirectUrl?: string;
  /** URL path where prefetch should fire on page load (e.g. "/account/settings").
   *  When the current page matches, PostHog session analysis is prefetched immediately
   *  instead of waiting for hover. Reduces perceived latency on the cancel flow. */
  prefetchUrl?: string;
  /** Callback when user clicks "Go Back" on done screen (re-engaged) */
  onRetained?: () => void;
  /** Callback when user clicks "Proceed with Cancellation" on done screen */
  onChurned?: () => void;
}
...
export interface InitiateResponse {
  ...
  agentId: string;
  /** Conversation token for voice agent (WebRTC) */
  conversationToken: string | null;
  /** ElevenLabs chat agent ID (text-only mode) */
  chatAgentId: string | null;
  /** Conversation token for chat agent (WebRTC) */
  chatConversationToken: string | null;
