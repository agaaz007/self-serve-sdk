...
export interface ElevenLabsAgentOptions {
  ...
  agentId: string;
  /** Conversation token for WebRTC connection (optional, for private agents) */
  ...
  /** Callback with the ElevenLabs conversation_id once the call is established */
...
export class ElevenLabsAgentHandler {
        ...
        onDisconnect: (details: any) => {
          ...
          if (!this.interviewCompleted) {
            if (this.conversationStarted) {
              console.log('[ElevenLabs] Conversation ended — showing done state');
              this.interviewCompleted = true;
              this.options.onInterviewComplete?.({ retained: false });
            } else {
              console.log('[ElevenLabs] Disconnected before conversation started — treating as error');
              this.interviewCompleted = true;
              this.emitError('Connection lost before conversation started', 'CONNECTION_CLOSED_EARLY');
            }
          }
        },
   ...
   * Used when SDK is unavailable or for text-only mode
   */
  private async connectDirectWebSocket(textOnly = false): Promise<void> {
    // WebSocket fallback connects via agentId directly (public agents only)
    const wsUrl = `wss://api.elevenlabs.io/v1/convai/conversation?agent_id=${this.options.agentId}`;
    console.log('[ElevenLabs] Connecting via WebSocket fallback:', wsUrl);
    ...
    return new Promise((resolve, reject) => {
      ...
      ws.onclose = (event) => {
        ...
        if (!this.interviewCompleted) {
          if (this.conversationStarted) {
            console.log('[ElevenLabs] WebSocket closed after conversation — auto-completing');
            this.interviewCompleted = true;
            this.options.onInterviewComplete?.({ retained: false });
          } else {
            console.log('[ElevenLabs] WebSocket closed before conversation started — treating as error');
            this.interviewCompleted = true;
            this.emitError('Connection closed before conversation started', 'CONNECTION_CLOSED_EARLY');
          }
        }
      };
    ...
    switch (data.type) {
      ...
      case 'ping':
        ...
        if (this.ws && this.ws.readyState === WebSocket.OPEN) {
          this.ws.send(JSON.stringify({ type: 'pong', event_id: data.ping_event?.event_id }));
        }
        break;

      case 'agent_tool_response':
        console.log('[ElevenLabs] Agent tool response:', JSON.stringify(data.agent_tool_response));
        break;
  ...
  async disconnect(): Promise<void> {
    // Prevent onclose/onDisconnect from firing callbacks during intentional cleanup
    this.interviewCompleted = true;
