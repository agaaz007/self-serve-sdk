...
interface ExtendedEmbedConfig extends EmbedConfig {
  ...
  /** Conversation token for private agents (WebRTC) */
  elevenLabsConversationToken?: string;
...
class ExitButton implements ExitButtonInstance {
  ...
  constructor(config: ExtendedEmbedConfig) {
    ...
    // Auto-prefetch if current page matches prefetchUrl
    if (config.prefetchUrl) {
      this.autoPrefetchIfPageMatches(config.prefetchUrl);
    }
  }

  /**
   * Auto-prefetch if the current page pathname matches the given URL path.
   * Fires immediately so PostHog analysis is cached before the user hovers or clicks cancel.
   */
  private autoPrefetchIfPageMatches(prefetchUrl: string): void {
    try {
      const currentPath = window.location.pathname;
      if (currentPath === prefetchUrl || currentPath.startsWith(prefetchUrl + '/')) {
        console.log('[ExitButton] Page matches prefetchUrl, auto-prefetching');
        this.prefetch();
      }
    } catch {
      // ignore — window.location may not be available in SSR
    }
  }
  ...
  async start(): Promise<void> {
    if (this.currentState !== 'closed' || this.starting) {
      return;
    }
    this.starting = true;
    ...
    this.createModal();
    this.modal?.open();
    const container = this.modal?.getContainer();
    if (container) this.applyThemeToContainer(container);
  ...
  private voiceConversationToken: string | null = null;
  private chatAgentIdResolved: string | null = null;
  private chatConversationToken: string | null = null;
  ...
  private async startElevenLabsAgentFlow(): Promise<void> {
    ...
    let conversationToken = this.config.elevenLabsConversationToken;

    // Use the API client to initiate session (fetches PostHog analytics + conversation token)
    if (this.config.backendUrl) {
      try {
        ...
        if (data.conversationToken) conversationToken = data.conversationToken;
        ...
        this.chatConversationToken = data.chatConversationToken || null;
    ...
    this.voiceConversationToken = conversationToken || null;

    console.log('[ExitButton] Voice agent:', this.voiceAgentId, '| Chat agent:', this.chatAgentIdResolved);

    // If we have no agent credentials at all, show error instead of broken permission screen
    if (!this.voiceAgentId && !this.chatAgentIdResolved) {
      console.error('[ExitButton] No agent credentials available — cannot start interview');
      this.handleError(new ExitButtonError('Unable to connect. Please try again later.', 'NO_AGENT_CREDENTIALS'));
      return;
    }
  ...
  private createAgentForMode(textOnly: boolean): void {
    const agentId = textOnly
      ...
      : this.voiceAgentId!;
    const conversationToken = textOnly
      ? (this.chatConversationToken || this.voiceConversationToken)
      : this.voiceConversationToken;

    console.log('[ExitButton] Creating agent for', textOnly ? 'text' : 'voice', 'mode, agentId:', agentId, 'token:', conversationToken ? 'yes' : 'none');
    ...
    this.elevenLabsAgent = new ElevenLabsAgentHandler({
      ...
      onConversationId: (conversationId) => {
        console.log('[ExitButton] ElevenLabs conversation_id:', conversationId);
        // Notify backend so it can link the ElevenLabs post-call webhook to our session
        if (this.sessionId && this.config.backendUrl) {
          fetch(`${this.config.backendUrl}/api/exit-session/${this.sessionId}/conversation`, {
            method: 'PATCH',
            headers: {
            ...
            // Non-critical: backend may not support this endpoint yet
            console.warn('[ExitButton] Could not link conversation_id to session:', e.message);
          });
        }
      },
      onInterviewComplete: () => {
        // Show done state immediately (spinner while waiting for webhook)
        this.setState('done');
        // Poll backend for the webhook-determined outcome
        this.pollForOutcome();
      },
      onError: (error) => {
        this.handleError(error);
  ...
  /**
   * Poll the backend for the webhook-determined outcome (retained/churned).
   * The ElevenLabs post-call webhook fires seconds after the call ends and
   * updates the session record. We poll for up to 5 seconds:
   * - retained → auto-navigate back to main page
   * - churned  → show done screen with two buttons
   * - timeout  → treat as churned and show two buttons
   */
  private async pollForOutcome(): Promise<void> {
    if (!this.config.backendUrl || !this.sessionId) {
      // No backend — fall back to showing both buttons with no outcome
    ...
    while (true) {
      ...
      if (this.currentState !== 'done' || !this.modal) return;

      try {
        const res = await fetch(
          `${this.config.backendUrl}/api/exit-session/${this.sessionId}`,
          {
        ...
        if (res.ok) {
          ...
          const outcome: string | undefined = session.outcome;
          console.log(`[ExitButton] Poll — outcome: ${outcome ?? 'pending'}`);

          if (outcome === 'retained' || outcome === 'churned') {
            console.log('[ExitButton] Webhook outcome received:', outcome);

            const sessionData = this.buildSessionData(outcome, {
              acceptedOffer: session.offers?.[0],
              churnRiskScore: outcome === 'retained' ? 35 : 72,
            });
            this.config.onComplete?.(sessionData);

            if (outcome === 'retained') {
              // User was retained — close modal and navigate back to main page
          ...
          console.warn(`[ExitButton] Poll HTTP ${res.status}`);
        }
      } catch (e) {
        if (this.pollController?.signal.aborted) return;
        console.warn('[ExitButton] Poll attempt failed:', e);
      }

      const remaining = deadline - Date.now();
      if (remaining <= 0) break;
      await new Promise(resolve => setTimeout(resolve, Math.min(INTERVAL_MS, remaining)));
      if (this.pollController?.signal.aborted) return;
    }
  ...
  private async acceptOffer(index: number): Promise<void> {
    if (this.mockMode) {
      ...
      await new Promise((resolve) => setTimeout(resolve, 800));
      const mockSession = this.buildSessionData('retained', { acceptedOffer: this.offers[index], churnRiskScore: 45 });
      this.config.onComplete?.(mockSession);
      this.setState('done');
      return;
    }

    await this.completeWithOutcome('retained', { acceptedOffer: this.offers[index] });
  }
  ...
  private async proceedWithCancellation(): Promise<void> {
    if (this.mockMode) {
      this.setState('completing');
      await new Promise((resolve) => setTimeout(resolve, 800));
      const mockSession = this.buildSessionData('churned', { churnRiskScore: 72 });
      this.config.onComplete?.(mockSession);
      this.redirectChurned();
      return;
    }

    await this.completeWithOutcome('churned');
    this.redirectChurned();
  }

  private redirectChurned(): void {
    this.close();
    if (this.config.onChurned) {
      this.config.onChurned();
    } else if (this.config.churnRedirectUrl) {
      window.location.href = this.config.churnRedirectUrl;
    }
  }
  ...
  private buildSessionData(
    ...
    options?: { acceptedOffer?: Offer; churnRiskScore?: number }
  ): Session {
    return {
      id: this.sessionId!,
      userId: this.config.userId,
      ...
      churnRiskScore: options?.churnRiskScore ?? 0,
  ...
  private async completeWithOutcome(
    ...
    try {
      ...
      const sessionData = this.buildSessionData(outcome, { acceptedOffer: options?.acceptedOffer });
      this.config.onComplete?.(sessionData);
      this.setState('done');
    } catch (error) {
      this.handleError(error as ExitButtonError);
  ...
  private cleanup(): void {
    ...
    this.voiceConversationToken = null;
    this.chatAgentIdResolved = null;
    this.chatConversationToken = null;
    this.posthogContext = undefined;
    this.dynamicVariablesResolved = undefined;
  }

  /**
   * Handle "Go Back" from done screen — user is re-engaged
   */
  private handleReturnHome(): void {
    this.close();
    if (this.config.onRetained) {
      this.config.onRetained();
    } else {
  ...
  /**
   * Handle "Proceed with Cancellation" from done screen — user wants to churn
   */
  private handleProceedCancelFromDone(): void {
    this.redirectChurned();
  }
  ...
  private applyTheme(theme: EmbedConfig['theme']): void {
    ...
    if (theme.primaryColor) {
      overrides['--exit-button-accent'] = theme.primaryColor;
      overrides['--exit-button-accent-rgb'] = this.hexToRgb(theme.primaryColor);
    }
    if (theme.primaryHoverColor) {
      overrides['--exit-button-accent-hover'] = theme.primaryHoverColor;
    }
    if (theme.backgroundColor) {
      overrides['--exit-button-overlay-bg'] = theme.backgroundColor;
    }
    if (theme.surfaceColor) {
      overrides['--exit-button-glass'] = theme.surfaceColor;
    }
    if (theme.textColor) {
      overrides['--exit-button-text'] = theme.textColor;
    }
    if (theme.textSecondaryColor) {
      overrides['--exit-button-text-secondary'] = theme.textSecondaryColor;
    }
    if (theme.errorColor) {
      overrides['--exit-button-error'] = theme.errorColor;
      overrides['--exit-button-error-rgb'] = this.hexToRgb(theme.errorColor);
    }
    if (theme.successColor) {
      overrides['--exit-button-success'] = theme.successColor;
      overrides['--exit-button-success-rgb'] = this.hexToRgb(theme.successColor);
    }
    if (theme.borderRadius) {
      overrides['--exit-button-radius'] = theme.borderRadius;
    }
    if (theme.fontFamily) {
      overrides['--exit-button-font'] = theme.fontFamily;
    }

    this.themeOverrides = overrides;
  }

  /** Apply stored theme overrides to the modal container element */
  private applyThemeToContainer(container: HTMLElement): void {
    for (const [prop, value] of Object.entries(this.themeOverrides)) {
...
function autoInit(): void {
  ...
  let theme: EmbedConfig['theme'] | undefined;
  if (script.dataset.theme) {
    try {
      theme = JSON.parse(script.dataset.theme);
    } catch {
      console.warn('ExitButton: data-theme must be valid JSON');
    }
  }
  ...
  const config: ExtendedEmbedConfig = {
    ...
    redirectUrl: script.dataset.redirectUrl,
    churnRedirectUrl: script.dataset.churnRedirectUrl,
    prefetchUrl: script.dataset.prefetchUrl,
    theme,
  };
